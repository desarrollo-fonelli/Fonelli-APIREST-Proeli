...
# Constantes locales
const K_SCRIPTNAME  = "listasprecios.php";
...
$ListaCodigo  = null;    // Id de la Lista de Precios
...
$arrPermitidos = array("Usuario", "ListaCodigo", "Pagina");
...
# Ejecuta la consulta 
try {
  $data = SelectListasPrecios($ListaCodigo);
 ...
 * 
 * @param int $ListaCodigo
 * @return array
 */
FUNCTION SelectListasPrecios($ListaCodigo)
{
  $where = "";

  # En caso necesario, hay que formatear los parametros que se van a pasar a la consulta
  if(isset($ListaCodigo)){
    if(is_numeric(trim($ListaCodigo))){
      $strListaCodigo = str_pad($ListaCodigo, 2,"0",STR_PAD_LEFT);
    } else {
      $strListaCodigo = str_pad($ListaCodigo, 2," ",STR_PAD_RIGHT);
    } 
  }
  ...
  $where = "WHERE a.t_tica = '10' AND t_gpo = '93' ";
  ...
  if(isset($ListaCodigo)){
    if(trim($ListaCodigo) == ""){
      ...
      $where .= "AND a.t_clave = :strListaCodigo ";
    }
  ...
  try {
    ...
    if(isset($ListaCodigo) && trim($ListaCodigo)!=""){
      $oSQL-> bindParam(":strListaCodigo", $strListaCodigo, PDO::PARAM_STR);
    }
...
FUNCTION CreaDataCompuesta( $data )
{
  ...
  foreach($data as $row){
    ...
    $oFila = [
      "ListaCodigo"     => $row["t_clave"],
      "ListaDescripc"   => $row["t_descr"]
    ];  
