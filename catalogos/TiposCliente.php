...
/**
 * Lista del Catalogo de Tipos de Cliente
 * --------------------------------------------------------------------------
 */
...
# Constantes locales
const K_SCRIPTNAME  = "tiposcliente.php";
...
$TipoCodigo  = null;    // Id del tipo de cliente
...
$arrPermitidos = array("Usuario", "TipoCodigo", "Pagina");
...
if (isset($_GET["TipoCodigo"])) {
  $TipoCodigo = $_GET["TipoCodigo"];
}
 ...
 * @return array
 */
FUNCTION SelectTipos($TipoCodigo)
{
  $where = "";

  # En caso necesario, hay que formatear los parametros que se van a pasar a la consulta
  if(isset($TipoCodigo)){
    if(is_numeric(trim($TipoCodigo))){
      $strTipoCodigo = str_pad($TipoCodigo, 2,"0",STR_PAD_LEFT);
    } else {
      $strTipoCodigo = str_pad($TipoCodigo, 2," ",STR_PAD_RIGHT);
    }    
  }
  ...
  if(isset($TipoCodigo)){
    if(trim($TipoCodigo) == ""){
      ...
      $where .= "AND a.t_gpo = :strTipoCodigo ";
    }
  ...
  $sqlCmd = "SELECT trim(a.t_gpo) t_gpo,trim(a.t_descr) t_descr
   FROM var020 a 
   $where 
  ...
  try {
    ...
    if(isset($TipoCodigo) && trim($TipoCodigo)!=""){
      $oSQL-> bindParam(":strTipoCodigo", $strTipoCodigo, PDO::PARAM_STR);
    }
...
FUNCTION CreaDataCompuesta( $data )
{
  ...
  foreach($data as $row){
    ...
    $oFila = [
      "TipoCodigo"     => $row["t_gpo"],
      "TipoDescripc"   => $row["t_descr"]
    ];  
