...
/**
 * Obtiene el último valor registrado en la tabla de paridades,
 * el cual se actualiza en el upload nocturno y a las 09 am
 * Creación: dRendon - 12.06.2026
 * -------------------------------------------------------------------------
 */
...
# Constantes locales
const K_SCRIPTNAME  = "GetParidades.php";
...
//$FechaDesde = null;     // No se requiere la fecha, se toman los valores que están en la tabla inv100
...
$arrPermitidos = array("TipoUsuario", "Usuario");
...
function SelectData()
{
  ...
  try {
    ...
    $sqlCmd = "SELECT p.ti_llave,p.ti_abrev,p.ti_descr,p.ti_parx 
      FROM inv100 p
      ORDER BY p.ti_llave";
    $oSQL = $conn->prepare($sqlCmd);
    $oSQL->execute();
...
function CreaDataCompuesta($data)
{
  ...
  if (count($data) > 0) {
    ...
    foreach ($data as $row) {
      $arrParidades[] = [
        "Codigo" => trim($row["ti_llave"]),
        "Abreviat" => trim($row["ti_abrev"]),
        "Descripc" => trim($row["ti_descr"]),
        "ValorCapturado" => floatval($row["ti_parx"])
      ];
    }
  }
