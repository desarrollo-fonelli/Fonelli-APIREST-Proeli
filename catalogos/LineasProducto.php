...
/**
 * Lista del Catalogo de Líneas de Producto
 * --------------------------------------------------------------------------
 */
...
# Constantes locales
const K_SCRIPTNAME  = "lineasproducto.php";
...
$LineaCodigo   = null;    // Id de la línea
...
$arrPermitidos = array("Usuario", "LineaCodigo", "Pagina");
...
if (isset($_GET["LineaCodigo"])) {
  $LineaCodigo = $_GET["LineaCodigo"];
}
 ...
 * 
 * @param string $LineaCodigo
 * @return array
 */
FUNCTION SelectLineas($LineaCodigo)
{
  $where = "";

  # En caso necesario, hay que formatear los parametros que se van a pasar a la consulta
  if(isset($LineaCodigo)){
    if(is_numeric(trim($LineaCodigo))){
      $strLineaCodigo = str_pad($LineaCodigo, 2,"0",STR_PAD_LEFT);
    } else {
      $strLineaCodigo = str_pad($LineaCodigo, 2," ",STR_PAD_RIGHT);
    }    
  }
  ...
  if(isset($LineaCodigo)){
    if(trim($LineaCodigo) == ""){
      ...
      $where .= "AND a.t_gpo = :strLineaCodigo ";
    }
  ...
  try {
    ...
    if(isset($LineaCodigo) && trim($LineaCodigo)!=""){
      $oSQL-> bindParam(":strLineaCodigo", $strLineaCodigo, PDO::PARAM_STR);
    }
