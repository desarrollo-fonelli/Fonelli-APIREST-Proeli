...
# Constantes locales
const K_SCRIPTNAME  = "listacategorias.php";
...
$CategoriaCodigo  = null; // Id de la Categoria
$Subcategoria = null;     // Id de Subcategoria
...
$arrPermitidos = array("Usuario", "CategoriaCodigo", "Subcategoria", "Pagina");
...
if (isset($_GET["CategoriaCodigo"])) {
  $CategoriaCodigo = $_GET["CategoriaCodigo"];
}
if (isset($_GET["Subcategoria"])) {
  if (isset($CategoriaCodigo)) {
    $Subcategoria = $_GET["Subcategoria"];
  } else {
    $mensaje = "Si incluye 'Subcategoria', debe indicar 'CategoriaCodigo'.";    // quité K_SCRIPTNAME del mensaje
    http_response_code(400);
    echo json_encode(["Code" => K_API_ERRPARAM, "Mensaje" => $mensaje]);
    exit;
  }
...
# Ejecuta la consulta 
try {
  $data = SelectCategorias($CategoriaCodigo, $Subcategoria);
 ...
 * 
 * @param string $CategoriaCodigo
 * @param string $Subcategoria
 * @return array
 */
function SelectCategorias($CategoriaCodigo, $Subcategoria)
{
  $where = "";
  ...
  /*
  if(isset($CategoriaCodigo)){
    if(is_numeric(trim($CategoriaCodigo))){
      $strCategoriaCodigo = str_pad($CategoriaCodigo, 2,"0",STR_PAD_LEFT);
    } else {
      $strCategoriaCodigo = str_pad($CategoriaCodigo, 2," ",STR_PAD_RIGHT);
    } 
  }
  */
  ...
  $where = "WHERE a.t_tica = '02' ";

  if (isset($CategoriaCodigo)) {
    if (trim($CategoriaCodigo) == "") {
      // Si se pasó un valor en blanco no devuelve ningun registro
      $where = "WHERE 1=2 ";
    } else {
      $where .= "AND a.t_gpo = :CategoriaCodigo ";
      ...
      if (isset($Subcategoria)) {
        if (trim($Subcategoria) == "") {
          ...
          if (trim($Subcategoria) == "*") {
            ...
            $where .= "AND a.t_clave = :Subcategoria ";
          }
        }
      } else {
        # $where .= "AND a.t_clave = ' ' ";
  ...
  try {
    ...
    if (isset($CategoriaCodigo) && trim($CategoriaCodigo) != "") {
      if ($where != "WHERE 1=2 ") {
        $oSQL->bindParam(":CategoriaCodigo", $CategoriaCodigo, PDO::PARAM_STR);
      }
    }

    if (isset($Subcategoria) && trim($Subcategoria) != "" && trim($Subcategoria) != "*") {
      $oSQL->bindParam(":Subcategoria", $Subcategoria, PDO::PARAM_STR);
    }
...
function CreaDataCompuesta($data)
{
  ...
  foreach ($data as $row) {
    ...
    $oFila = [
      "CategoriaCodigo"   => $row["t_gpo"],
      "Subcategoria"      => $row["t_clave"],
      "CategoriaDescripc" => $row["t_descr"]
    ];
