...
/**
 * Lista del Catalogo de Tipos de Cartera
 * --------------------------------------------------------------------------
 */
...
# Constantes locales
const K_SCRIPTNAME  = "tiposcartera.php";
...
$CarteraCodigo  = null;    // Id del tipo de Cartera
...
$arrPermitidos = array("Usuario", "CarteraCodigo", "Pagina");
...
if (isset($_GET["CarteraCodigo"])) {
  $CarteraCodigo = $_GET["CarteraCodigo"];
}
...
# Ejecuta la consulta 
try {
  $data = SelectCarteras($CarteraCodigo);
 ...
 * 
 * @param int $CarteraCodigo
 * @return array
 */
FUNCTION SelectCarteras($CarteraCodigo)
{
  $where = "";
  ...
  if(isset($CarteraCodigo)){
    /*
    if(is_numeric(trim($CarteraCodigo))){
      $strCarteraCodigo = str_pad($CarteraCodigo, 2,"0",STR_PAD_LEFT);
    } else {
      $strCarteraCodigo = str_pad($CarteraCodigo, 2," ",STR_PAD_RIGHT);
    } 
    */
    $strCarteraCodigo = str_pad($CarteraCodigo, 1," ",STR_PAD_LEFT);
  }
  ...
  if(isset($CarteraCodigo)){
    if(trim($CarteraCodigo) == ""){
      ...
      $where .= "AND a.t_clave = :strCarteraCodigo ";
    }
  ...
  $sqlCmd = "SELECT trim(a.t_clave) t_clave,trim(a.t_descr) t_descr
   FROM var020 a 
   $where 
  ...
  try {
    ...
    if(isset($CarteraCodigo) && trim($CarteraCodigo)!=""){
      $oSQL-> bindParam(":strCarteraCodigo", $strCarteraCodigo, PDO::PARAM_STR);
    }
...
FUNCTION CreaDataCompuesta( $data )
{
  ...
  foreach($data as $row){
    ...
    $oFila = [
      "CarteraCodigo"     => $row["t_clave"],
      "CarteraDescripc"   => $row["t_descr"]
    ];  
