...
$Status         = null;     // Status del artículo
$BuscarSemejantes = "N";    // Si se busca artículos semejantes (S=Si, N=No)
...
if (isset($_GET["BuscarSemejantes"])) {
  $BuscarSemejantes = $_GET["BuscarSemejantes"];
  if (! in_array($BuscarSemejantes, ["S", "N"])) {
    $mensaje = "Valor '" . $BuscarSemejantes . "NO permitido para 'BuscarSemejantes'";
    http_response_code(400);
    echo json_encode(["Code" => K_API_ERRPARAM, "Mensaje" => $mensaje]);
    exit;
  }
...
function SelectData(
  ...
  $strLineaPTDesde  = str_pad($LineaPTDesde, 2, "0", STR_PAD_LEFT);
  $strLineaPTHasta  = str_pad($LineaPTHasta, 2, "0", STR_PAD_LEFT);
  ...
  $where = "WHERE itm.c_lin >= :LineaPTDesde AND itm.c_lin <= :LineaPTHasta ";

  if (isset($ItemCode) && trim($ItemCode) <> "") {

    if ($BuscarSemejantes == "N") {
      $where .= "AND itm.c_clave = :ItemCode ";
    } else {
      $ItemCode = "%" . trim($ItemCode) . "%";  // Si se busca semejantes, se usa LIKE
      $where .= "AND itm.c_clave ILIKE :ItemCode ";
    }
  }

  if (isset($Status) && $Status <> "T") {
    $where .=  "AND itm.c_status = :Status ";
  }
  ...
  try {
    ...
    $sqlCmd = "SELECT itm.c_lin,itm.c_clave,itm.c_descr,0.00 as precio,
      COALESCE(cli.cc_tparid, 'N') paridadtipo,
      COALESCE(cli.cc_tipoli, '11') listapreccode
    FROM inv010 itm
    LEFT JOIN cli010 cli ON cli.cc_num = :ClienteCodigo AND cli.cc_fil = :ClienteFilial
    $where 
    ORDER BY itm.c_lin,itm.c_clave LIMIT 500";

    // Prepara la consulta SQL
    $oSQL = $conn->prepare($sqlCmd);
    $oSQL->bindParam(":LineaPTDesde", $strLineaPTDesde, PDO::PARAM_STR);
    $oSQL->bindParam(":LineaPTHasta", $strLineaPTHasta, PDO::PARAM_STR);
    $oSQL->bindParam(":ClienteCodigo", $strClienteCodigo, PDO::PARAM_STR);
    $oSQL->bindParam(":ClienteFilial", $strClienteFilial, PDO::PARAM_STR);
    if (isset($ItemCode) && trim($ItemCode) <> "") {
  ...
  if (count($data) > 0) {
    ...
    foreach ($data as $row) {
      ...
      $contenido[] = array(
        ...
        "PrecioVenta" => floatval($row["precio"]),
        "ParidadTipo" => $row["paridadtipo"],
        "ListaPrecCode" => $row["listapreccode"]
      );
    }
  }
